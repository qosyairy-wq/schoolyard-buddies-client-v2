/* Schoolyard Buddies v3.0.2 — universal fullscreen UI controller */
(() => {
  'use strict';
  ...
  const startScreenVisible = () => styleVisible(document.getElementById('start-screen'));
  const gameplayReady = () => {
    if (startScreenVisible()) return false;
    const canvas = document.querySelector('#canvas-container canvas, #game-canvas, canvas');
    return !!canvas && styleVisible(canvas);
  };
  ...
  function refresh() {
    const roots = new Set();
    if (gameplayReady()) {
      document.querySelectorAll(ROOT_SELECTORS.join(',')).forEach((el) => {
        if (styleVisible(el)) roots.add(el);
      });
    }

    document.querySelectorAll('.' + ACTIVE_CLASS).forEach((el) => {
      if (!roots.has(el)) el.classList.remove(ACTIVE_CLASS);
    });
  ...
  const start = () => {
    ...
    scheduleRefresh();
    setInterval(scheduleRefresh, 350);
  };
